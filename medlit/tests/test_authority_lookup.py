...
Tests the matching logic for Wikidata and other ontology lookups,
and UMLS type validation (validate_umls_type).
"""

import pytest

from medlit.pipeline.authority_lookup import CanonicalIdLookup, validate_umls_type


class TestWikidataLabelMatching:
    """Test the Wikidata label matching logic."""
    ...
    def test_exact_match(self, lookup):
        ...
        assert lookup._wikidata_label_matches("breast cancer", "Breast Cancer")  # pylint: disable=protected-access

    def test_term_contained_in_label(self, lookup):
        """Term contained in label should succeed."""
        assert lookup._wikidata_label_matches("mitochondria", "<B>Mitochondria</B> (song)")  # pylint: disable=protected-access

    def test_label_contained_in_term(self, lookup):
        """Label contained in term should succeed."""
        assert lookup._wikidata_label_matches("breast cancer syndrome", "Breast Cancer")  # pylint: disable=protected-access

    def test_label_starts_with_term(self, lookup):
        """Label starting with term should succeed."""
        assert lookup._wikidata_label_matches("breast", "Breast cancer")  # pylint: disable=protected-access
    ...
    def test_common_prefix_singular_plural(self, lookup):
        ...
        assert lookup._wikidata_label_matches("mitochondria", "<B>Mitochondrion</B>")  # pylint: disable=protected-access

    def test_html_tags_stripped(self, lookup):
        """HTML bold tags should be stripped from labels."""
        assert lookup._wikidata_label_matches("mitochondria", "<B>Mitochondria</B>")  # pylint: disable=protected-access
        assert lookup._wikidata_label_matches("cancer", "<B>Cancer</B>")  # pylint: disable=protected-access

    def test_case_insensitive(self, lookup):
        """Matching should be case-insensitive."""
        assert lookup._wikidata_label_matches("BRCA1", "brca1")  # pylint: disable=protected-access
        assert lookup._wikidata_label_matches("brca1", "BRCA1")  # pylint: disable=protected-access
    ...
    def test_garbage_match_insect(self, lookup):
        ...
        assert not lookup._wikidata_label_matches("HER2-enriched", "Insect")  # pylint: disable=protected-access

    def test_garbage_match_animal(self, lookup):
        """Garbage match 'basal-like' → 'Animal' should fail."""
        assert not lookup._wikidata_label_matches("basal-like", "Animal")  # pylint: disable=protected-access

    def test_unrelated_terms(self, lookup):
        """Completely unrelated terms should fail."""
        assert not lookup._wikidata_label_matches("diabetes", "Python programming")  # pylint: disable=protected-access
        assert not lookup._wikidata_label_matches("gene", "Music")  # pylint: disable=protected-access
    ...
    def test_substring_match_allowed(self, lookup):
        ...
        assert lookup._wikidata_label_matches("test", "Testing framework")  # pylint: disable=protected-access
        # "car" is contained in "cardinal" - this is accepted
        # (for medical terms, coincidental substrings are rare)
        assert lookup._wikidata_label_matches("car", "Cardinal")  # pylint: disable=protected-access

    def test_no_overlap_fails(self, lookup):
        """Terms with no overlap should fail."""
        # No substring match, no common prefix
        assert not lookup._wikidata_label_matches("xyz", "abc")  # pylint: disable=protected-access
        assert not lookup._wikidata_label_matches("hello", "world")  # pylint: disable=protected-access
